...
title: Brigade के साथ त्वरित प्रारंभ
description: Brigade के साथ त्वरित प्रारंभ
section: परिचय
weight: 3
aliases:
  - /quickstart.md
...
यह त्वरित प्रारंभ Brigade का व्यापक परिचय प्रस्तुत करता है. आप ब्रिगेड को डिफ़ॉल्ट कॉन्फिगरेशन के साथ लोकल, डेवलपमेंट-ग्रेड cluster पे इंस्टाल करेंगे, प्रोजेक्ट और इवेंट बनानांगे, ब्रिगेड को इवेंट हैंडल करते हुए देखेंगे, और फिर वाह चिज रिमूव करेंगे जो आपको नहीं चाहिए।
...
* [इंस्टॉल Brigade](#install-brigade)
  * [Brigade CLI की इंस्टालेशन](#install-the-brigade-cli)
...
* [परीक्षा लेना](#trying-it-out)
  * [Brigade में लॉगिन](#log-into-brigade)
...
> ⚠️ इस गाइड में जो डिफ़ॉल्ट कॉन्फ़िगरेशन है, वो केवल Brigade को लोकल, 
> डेवलपमेंट-ग्रेड cluster पर देखने के लिए है, ना की कोई _any_ shared 
> cluster के लिए-- _विशेष रूप से प्रोडक्शन वाले क्लस्टर्स के लिए_।
> हमारा [डेवलपमेंट गाइड](/topics/operators/deploy/) देखे shared या 
>production clusters के लिए। हमने ये निर्देश स्थानीय cluster पे टेस्ट 
>किया है।
> [KinD](https://kind.sigs.k8s.io/)

* एक local, _development-grade_ Kubernetes v1.16.0+ cluster
...
प्रसिद्ध [Homebrew](https://brew.sh/) पैकेज मैनेजर सबसे सुविधाजनक विधि प्रदान करता है जिसे Brigade CLI Mac पर स्थापित हो सके:
...
### Server-Side Components की इंस्टालेशन

अपने स्थानीय, डेवलपमेंट-ग्रेड cluster पर सर्वर-साइड कंपोनेंट्स को स्थापित करने के लिए:

1. हेल्म का प्रयोगात्मक OCI समर्थन सक्षम करें:
...
1.Brigade को डिफ़ॉल्ट कॉन्फ़िगरेशन के साथ स्थापित करने के लिए निम्नलिखित कमांड चलाएँ:
...
चूंकि आप Brigade को स्थानीय रूप से चला रहे हैं, इसलिए Brigade API उपलब्ध बनाने के लिए पोर्ट फ़ॉरवर्डिंग का उपयोग करें स्थानीय नेटवर्क इंटरफेस के माध्यम से: 
...
Brigade को रूट यूजर के तौर पर प्रमाणित करने के लिए आपको auto-generated रूट यूजर पासवर्ड हैसिल करनी होगी:
...
`--insecure` ध्वज `brig login` को स्व-हस्ताक्षरित प्रमाण पत्र को अनदेखा करने का निर्देश देता है जो
 हमारे Brigade की स्थानीय स्थापना द्वारा उपयोग किया गया है।
...
1. प्रारंभ से एक प्रोजेक्ट परिभाषा बनाने के बजाय, हम  प्रक्रिया को `brig init` कमांड से आगे लेके जाएंगे:
    ...
    यह `exec` इवेंट्स को सबस्क्राइब करता है `brigade.sh/cli` नामक एक सोर्स से एमिट कार्ति है| (इस प्रकार की इवेंट आसानी से बनाया जाता है CLI का उपयोग करके, इसलिए यह डेमो उद्देश्यों के लिए बहुत अच्छा है।) जब ऐसा कोई इवेंट हो, तब एम्बेडेड स्क्रिप्ट एक्जीक्यूट होता है। स्क्रिप्ट खुद ही खंडित होता है इवेंट के प्रकार के हिसाब से| `exec` इवेंट के लिए `brigade.sh/cli` नाम सोर्स से, ये स्क्रिप्ट एक सिंपल "Hello
...
1. पिछली कमांड ने केवल एक टेम्पलेट से एक प्रोजेक्ट परिभाषा उत्पन्न की। हमे
    परियोजना निर्माण को पूरा करने के लिए अभी भी Brigade को इस परिभाषा को अपलोड करने की आवश्यकता है:

    ```shell
    $ brig project create --file .brigade/project.yaml
    ```

1. यह देखने के लिए कि Brigade को अब इस प्रोजेक्ट के बारे में पता है, `brig project list` का उपयोग करें:
...
> ⚠️ डिफ़ॉल्ट रूप से, ब्रिगेड के scheduler नए प्रोजेक्ट के लाई स्कैन करता है प्रति तीस सेकंड में।
> अगर Brigade आपके पहले इवेंट को हैंडल करने में धीमी है, तो यह इसकी वजह हो सकती है।

## सफ़ाई

अगर आपको आपकी Brigade की स्थापना रखना है, तो यह कमांड रन करे तकी जो उदाहरण प्रोजेक्ट अभी बनाया गया है, वह हट जाए:

```shell
$ brig project delete --id first-project
```

नहीं तो, आप _all_ resources जो अभी बनायी गई है, वो हटा सकता है:
...
अभी आपको पता है की Brigade को लोकल, डेवलपमेंट-ग्रेड cluster में कैसे इंस्टॉल करते हैं, कैसे प्रोजेक्ट डिफाइन करते है और कैसे एक मैन्युअल इवेंट बनाते हैं।
जारी रहे
...
Cluster node पे डिस्क स्पेस कम होना एक सामान्य वजह है Brigade की डिप्लॉयमेंट के असफल होने का। MacOS या Windows पर स्थानीय, विकास-ग्रेड cluster में, यह हो सकता है
क्योंकि अपर्याप्त डिस्क स्थान Docker Desktop को आवंटित किया गया है या
आवंटन स्थान लगभग पूरा हो चुका है।अगर ये मामला है, तो यह Brigade के MongoDB या ActiveMQ Artemis pods के logs को जांचकर स्पश्त हो जाएगा।
अगर logs में ऐसा कुछ संदेश शमील है- "No space left on device" or "Disk Full!",तो आपको डिस्क स्पेस को फ्री करके फिर से इंस्टालेशन कोशिश करना होगा|
...
`-k`) flag. यह फ्लैग ज़रुरी है क्यों की डिफ़ॉल्ट कॉन्फ़िगरेशन जो ईस क्विकस्टार्ट/त्वरित प्रारंभ में उपयोग किया गया समलैंगिक है, वो स्वयं हस्ताक्षरित प्रमाणपत्र उपयोग करता है।